 ...
 $(function(){
    $('#faq').on('click', '.user', function(){
	 ...
	 $("#faq").on('click', '.mark', function(){
		 ...
		 $.ajax({
			 type: "POST",
			 url: "<?php echo Yii::app()->createUrl("/admin/feedback/ajax"); ?>",
			 data: {item: $(this).data('item'), action: "published"},
			 dataType: "json",
...
<table id="faq">
    ...
    <tr id="question-<?php echo $item->id; ?>" class="row<?php echo $item->id % 2 ? 0 : 1; ?>">
        ...
        <td class="title"><?php echo CHtml::link($item->username . " (". $item->mail. ")", "javascript:;", array('class' => 'user', 'data-item' => $item->id)); ?></td>
        <td class="actions">
	        <div class="mark <?php echo !$item->published ? 'marked' : 'unmarked'; ?>" data-item="<?php echo $item->id; ?>"></div>
	        <?php echo CHtml::ajaxLink(' ', $this->createUrl('feedback/delete', array('id'=>$item->id)),
		        array(
		            'type'=>'post',
		            'data'=>array('ajax'=>1),
