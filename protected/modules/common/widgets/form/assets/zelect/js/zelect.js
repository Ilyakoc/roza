/*
  Файл модифицирован.
  Добавлен параметр:
  	searchPos: "bottom", "top" позиция отображения поля поиска. По умолчанию "top".

  zelect-0.0.9
  @see https://github.com/mtkopone/zelect
  ...
  $.fn.zelect = function(opts) {
    ...
    return this.each(function() {
      ...
      $zelect.insertAfter($select)
        .append($selected)
        .append(
        	(opts.searchPos=="bottom")
        		? $dropdown.append($list).append($('<div>').addClass('zearch-container').append($search).append($noResults))
